...
class Geometry
{
 public:
  ...
  static geom::Point3 TriangleCentroid(
    const geom::Point3& _p0,
    const geom::Point3& _p1,
    const geom::Point3& _p2);
  ...
  static geom::Vector2 Normalize(const geom::Vector2& _v);

  /// \brief Normalise a Vector3 (i.e. ensure it has unit length)
  ///
  /// \param[in] _v     The vector to normalise.
  /// \return           The normalized vector.
  static geom::Vector3 Normalize(const geom::Vector3& _v);
  ...
  static geom::Vector3 Normal(
    const geom::Point3& _v0,
    const geom::Point3& _v1,
    const geom::Point3& _v2);
  ...
  static geom::Point3 HorizontalIntercept(
  ...
  /// \param[in] _face    A face index for the surface mesh.
  /// \return             A triangle.
  static geom::Triangle MakeTriangle(const geom::Mesh& _mesh,
      geom::FaceIndex _face);
  ...
  static std::shared_ptr<geom::RayMeshQuery> MakeAABBTree(
      const geom::Mesh& _mesh);
